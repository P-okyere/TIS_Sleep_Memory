...
plot_dir_corrected = fullfile(out_path, 'topoplot_significant');
plot_dir_null      = fullfile(out_path, 'topoplot_not_significant');
...
T = all_so_trial_data( ...
    ...
    ~isnan(all_so_trial_data.MeanPTP), :);
...
fprintf('\nParticipants: %d | Conditions: %d\n', nP, n_conds);

%% ====================================================
%  STEP 1 — Channel-wise LME (observed F values)
%% ====================================================
...
for c = 1:n_channels
    ...
    if height(tbl) >= 10 && numel(unique(tbl.Participant)) > 1
        try
            lme          = fitlme(tbl, ...
                'MeanPTP ~ MarkerType + Trial + (1|Participant)');
            aov          = anova(lme);
            fvals_obs(c) = aov.FStat(3);
            pvals_obs(c) = aov.pValue(3);
        catch
...
fname    = 'SO_PTP_ANOVA_clusterperm';
