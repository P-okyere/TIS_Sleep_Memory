...
plot_dir_corrected = fullfile(out_root, 'topoplot_significant');
plot_dir_null      = fullfile(out_root, 'topoplot_not_significant');
...
%% ============================
%  FILTER DATA — positive density only (Gamma requires > 0)
%% ============================
T_all = all_so_trial_data( ...
    ismember(all_so_trial_data.MarkerType,  marker_set) & ...
    ismember(all_so_trial_data.SleepStage,  sleep_stages_use), :);

% Keep only positive density trials
...
fprintf('\nAll trials: %d | Positive density trials: %d (%.1f%%)\n', ...
...
%  Tests: given an SO was detected, does rate differ by condition?
%% ====================================================
...
for c = 1:n_channels
    ...
    if height(tbl) >= 10 && ...
       ...
       numel(unique(string(tbl.MarkerType))) == n_conds
        try
            glme = fitglme(tbl, ...
...
%% ====================================================
%  STEP 4 — Permutation (condition label shuffle on positive-density means)
%% ====================================================
...
fname    = 'SO_density_Gamma_posonly_clusterperm';
...
% 1, 2, ... in the order found). The bar plot script loads these files
% rather than hardcoding channel lists.
sig_rank = 0;
for cl = 1:numel(clusters_obs)
    cl_pval = mean(perm_max_mass >= masses_obs(cl));
