...
min_cluster_size = 2;      % minimum channels to form a cluster
...
%% ============================
%  FIGURE SIZE — matches fast-spindle topoplot script
%% ============================
...
for s = 1:numel(spindle_types)
    ...
    for perm = 1:n_perm
        ...
        for c = 1:n_channels
            ...
            % One-way repeated measures F on flipped data
    ...
    for j = 1:numel(layout.label)
        ...
        if ~isempty(idx)
            ...
            if ismember(chan_lbl, all_sig_channels)
                % White = significant cluster (cluster-corrected)
